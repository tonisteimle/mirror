/**
 * Events Fixture Runner — Schicht 1 (Golden Files)
 *
 * Reads events subdirectory under tests/fixtures/events/, compiles
...
function normalizeHTML(html: string): string {
  ...
  // Pretty-print: events tag on its own line, indented by depth.
  return prettyHTML(stripped)
}

function prettyHTML(html: string): string {
  // Tokenize on `<…>` and text. Walk and indent.
  const events: Array<{ type: 'open' | 'close' | 'self' | 'text'; value: string }> = []
  let i = 0
  while (i < html.length) {
    if (html[i] === '<') {
      ...
      if (tag.startsWith('</')) events.push({ type: 'close', value: tag })
      else if (tag.endsWith('/>')) events.push({ type: 'self', value: tag })
      else events.push({ type: 'open', value: tag })
      i = end + 1
    } else {
      const next = html.indexOf('<', i)
      const text = html.slice(i, next === -1 ? html.length : next)
      if (text.trim()) events.push({ type: 'text', value: text.trim() })
      i = next === -1 ? html.length : next
    }
  }
  const out: string[] = []
  let depth = 0
  for (const tk of events) {
...
describe('Events Fixtures (Schicht 1 — Golden Files)', () => {
