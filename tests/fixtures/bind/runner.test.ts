/**
 * Bind Fixture Runner — Schicht 1 (Golden Files)
 *
 * Reads bind subdirectory under tests/fixtures/bind/, compiles
...
function normalizeHTML(html: string): string {
  ...
  // Pretty-print: bind tag on its own line, indented by depth.
  return prettyHTML(stripped)
}

function prettyHTML(html: string): string {
  // Tokenize on `<…>` and text. Walk and indent.
  const bind: Array<{ type: 'open' | 'close' | 'self' | 'text'; value: string }> = []
  let i = 0
  while (i < html.length) {
    if (html[i] === '<') {
      ...
      if (tag.startsWith('</')) bind.push({ type: 'close', value: tag })
      else if (tag.endsWith('/>')) bind.push({ type: 'self', value: tag })
      else bind.push({ type: 'open', value: tag })
      i = end + 1
    } else {
      const next = html.indexOf('<', i)
      const text = html.slice(i, next === -1 ? html.length : next)
      if (text.trim()) bind.push({ type: 'text', value: text.trim() })
      i = next === -1 ? html.length : next
    }
  }
  const out: string[] = []
  let depth = 0
  for (const tk of bind) {
...
describe('Bind Fixtures (Schicht 1 — Golden Files)', () => {
