/**
 * Layout Fixture Runner — Schicht 1 (Golden Files)
 *
 * Reads layout subdirectory under tests/fixtures/layout/, compiles
...
function normalizeHTML(html: string): string {
  ...
  // Pretty-print: layout tag on its own line, indented by depth.
  return prettyHTML(stripped)
}

function prettyHTML(html: string): string {
  // Tokenize on `<…>` and text. Walk and indent.
  const layout: Array<{ type: 'open' | 'close' | 'self' | 'text'; value: string }> = []
  let i = 0
  while (i < html.length) {
    if (html[i] === '<') {
      ...
      if (tag.startsWith('</')) layout.push({ type: 'close', value: tag })
      else if (tag.endsWith('/>')) layout.push({ type: 'self', value: tag })
      else layout.push({ type: 'open', value: tag })
      i = end + 1
    } else {
      const next = html.indexOf('<', i)
      const text = html.slice(i, next === -1 ? html.length : next)
      if (text.trim()) layout.push({ type: 'text', value: text.trim() })
      i = next === -1 ? html.length : next
    }
  }
  const out: string[] = []
  let depth = 0
  for (const tk of layout) {
...
describe('Layout Fixtures (Schicht 1 — Golden Files)', () => {
