/**
 * Actions Fixture Runner — Schicht 1 (Golden Files)
 *
 * Reads actions subdirectory under tests/fixtures/actions/, compiles
...
function normalizeHTML(html: string): string {
  ...
  // Pretty-print: actions tag on its own line, indented by depth.
  return prettyHTML(stripped)
}

function prettyHTML(html: string): string {
  // Tokenize on `<…>` and text. Walk and indent.
  const actions: Array<{ type: 'open' | 'close' | 'self' | 'text'; value: string }> = []
  let i = 0
  while (i < html.length) {
    if (html[i] === '<') {
      ...
      if (tag.startsWith('</')) actions.push({ type: 'close', value: tag })
      else if (tag.endsWith('/>')) actions.push({ type: 'self', value: tag })
      else actions.push({ type: 'open', value: tag })
      i = end + 1
    } else {
      const next = html.indexOf('<', i)
      const text = html.slice(i, next === -1 ? html.length : next)
      if (text.trim()) actions.push({ type: 'text', value: text.trim() })
      i = next === -1 ? html.length : next
    }
  }
  const out: string[] = []
  let depth = 0
  for (const tk of actions) {
...
describe('Actions Fixtures (Schicht 1 — Golden Files)', () => {
