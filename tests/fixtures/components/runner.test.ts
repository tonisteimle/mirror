/**
 * Component Fixture Runner — Schicht 1 (Golden Files)
 *
 * Reads each subdirectory under tests/fixtures/components/, compiles
...
function extractUserCode(code: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // First user comment / element declaration follows.
    if (
      start === -1 &&
      lines[i].match(/^\s*\/\/\s+\w/) &&
      i > 0 &&
      lines[i - 1].trim() === '' &&
      lines[i + 1] &&
      lines[i + 1].match(/^\s*const\s+node_\d+\s*=/)
    ) {
      start = i
    }
...
function normalizeHTML(html: string): string {
  ...
  // Pretty-print: each tag on its own line, indented by depth.
...
describe('Component Fixtures (Schicht 1 — Golden Files)', () => {
