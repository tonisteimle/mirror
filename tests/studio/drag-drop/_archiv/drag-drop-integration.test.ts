...
describe('Target Detection with DOMAdapter', () => {
  ...
  describe('findClosestTarget with hierarchy', () => {
    it('finds parent container when starting from leaf element', () => {
      ...
      const text = createTestElement('span', { [NODE_ID_ATTR]: 'text-1' }, { mirrorName: 'Text' })
      const container = createTestElement('div', { [NODE_ID_ATTR]: 'container-1' }, {}, [text])
      document.body.appendChild(container)
...
describe('Complete Drag-Drop Flow', () => {
  ...
  describe('palette drop to flex container', () => {
    it('completes full flow: detect → calculate → execute', () => {
      ...
      const targetElement = createTestElement('div', { [NODE_ID_ATTR]: 'flex-container' }, {})
  ...
  describe('palette drop to positioned container', () => {
    it('completes full flow with absolute positioning', () => {
      ...
      mockAdapter.setBoundingClientRectForElement(targetElement, createMockRect(100, 100, 400, 300))
  ...
  describe('canvas element move', () => {
    it('handles moving element within same container', () => {
      ...
      const child = createTestElement('div', { [NODE_ID_ATTR]: 'child-1' }, { mirrorName: 'Frame' })
      const targetElement = createTestElement('div', { [NODE_ID_ATTR]: 'container-1' }, {}, [child])
      container.appendChild(targetElement)
