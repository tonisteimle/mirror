...
const mockDOMAdapter = {
  getBoundingClientRect: (element: HTMLElement) => {
    ...
    return rect || { x: 0, y: 0, width: 100, height: 100, top: 0, left: 0, right: 100, bottom: 100, toJSON: () => ({}) }
  },
...
describe('Coordinate System Consistency', () => {
  describe('getSiblingRects coordinate system', () => {
    ...
    it('FIXED: returns viewport coords consistently with and without layoutInfo', () => {
      ...
      const rectsWithLayoutInfo = getSiblingRects(container, null, 'data-node-id', layoutInfo, mockDOMAdapter)

      // Get sibling rects WITHOUT layoutInfo
      const rectsWithoutLayoutInfo = getSiblingRects(container, null, 'data-node-id', undefined, mockDOMAdapter)
  ...
  describe('getChildRects coordinate system', () => {
    it('FIXED: returns viewport coords consistently with and without layoutInfo', () => {
      ...
      const rectsWithLayoutInfo = getChildRects(container, 'data-node-id', layoutInfo, mockDOMAdapter)

      // Get child rects WITHOUT layoutInfo
      const rectsWithoutLayoutInfo = getChildRects(container, 'data-node-id', undefined, mockDOMAdapter)
  ...
  describe('getContainerRect coordinate system', () => {
    it('FIXED: always returns viewport coordinates (ignores layoutInfo)', () => {
      ...
      const rectWithLayoutInfo = getContainerRect(element, layoutInfo, 'data-node-id', mockDOMAdapter)

      // Without layoutInfo: viewport
      const rectWithoutLayoutInfo = getContainerRect(element, undefined, 'data-node-id', mockDOMAdapter)
