...
// Note: jsdom does not resolve CSS custom properties via getComputedStyle —
// `var(--primary-bg)` shows up unmodified. The browser stack
// (studio/test-api/suites/integration/component-token.test.ts) covers the
// rgb-comparison cases in real Chrome; we skip them here so the jsdom
// mirror stays green.
...
runTestCases('Component + Token', componentTokenTests)
