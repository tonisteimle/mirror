...
export const buttonMinWidthTests: TestCase[] = describe('Button Min-Width Defaults', [
  ...
  // The width-via-getBoundingClientRect tests in this file need a real
  // layout engine. jsdom returns 0 for layout; the browser stack
  // (studio/test-api/suites/primitives/defaults.test.ts) covers these
  // with Chrome — we skip them here so the jsdom mirror stays green.
  testWithSetupSkip(
  ...
  testWithSetupSkip(
    'Button with "OK" is at least 36px wide',
    'Button "OK"',
    async (api: TestAPI) => {
      api.assert.exists('node-1')
      const element = document.querySelector('[data-mirror-id="node-1"]') as HTMLElement
  ...
  testWithSetupSkip(
    'Button with "No" is at least 36px wide',
    'Button "No"',
    async (api: TestAPI) => {
      api.assert.exists('node-1')
      const element = document.querySelector('[data-mirror-id="node-1"]') as HTMLElement
  ...
  testWithSetupSkip(
    'Button with pad 0 still has minw',
    'Button "X", pad 0',
    async (api: TestAPI) => {
      api.assert.exists('node-1')
      const element = document.querySelector('[data-mirror-id="node-1"]') as HTMLElement
...
runTestCases('Button Min-Width Defaults', buttonMinWidthTests)
