/**
 * IconPicker — material + Lucide icon picker with category filtering and search
 *
...
import { ICONS, getIconsByCategory, getCategories } from './icon-data'
...
export function setGlobalIconPickerCallback(onSelect: (value: string) => void): void {
  if (globalIconPicker) {
    ;(globalIconPicker as unknown as { callbacks: PickerCallbacks }).callbacks.onSelect = onSelect
  }
}
