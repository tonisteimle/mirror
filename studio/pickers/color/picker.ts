/**
 * ColorPicker — palette + custom-hex picker with keyboard navigation
 *
...
import { DEFAULT_PALETTES, parseColor, isLightColor, type ColorPalette } from './palette'
 ...
 * Convenience: creates and positions a color picker, wires click-outside to close.
 */
...
export function initColorPickerGlobalAPI(): void {
  ;(window as unknown as Record<string, unknown>).showColorPickerForProperty =
    showColorPickerForProperty
  ;(window as unknown as Record<string, unknown>).hideGlobalColorPicker = hideGlobalColorPicker
}
