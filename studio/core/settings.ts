...
export type AgentType = 'openrouter' | 'claude-cli' | 'anthropic-sdk'
...
export interface AgentSettings {
  ...
  /** Anthropic model (for anthropic-sdk) */
...
// In-memory state
let currentAgentSettings: AgentSettings = { ...DEFAULT_AGENT }
...
export const agentSettings = {
   ...
   */
  set(settings: Partial<AgentSettings>): void {
    currentAgentSettings = { ...currentAgentSettings, ...settings }
    events.emit('agent:changed', currentAgentSettings)
  },
  ...
  /**
   * Check if current agent is configured
   */
  isConfigured(): boolean {
    ...
    switch (settings.type) {
      ...
      case 'claude-cli':
        return true // Always available if Tauri is present
      default:
        return false
    }
  },
   ...
   */
  load(settings: Partial<AgentSettings>): void {
    currentAgentSettings = { ...DEFAULT_AGENT, ...settings }
    events.emit('agent:changed', currentAgentSettings)
  },
   ...
   */
  reset(): void {
    currentAgentSettings = { ...DEFAULT_AGENT }
    events.emit('agent:changed', currentAgentSettings)
  },
}
