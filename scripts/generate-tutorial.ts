...
function highlightMirrorCode(code: string): string {
  ...
  html = html.replace(
    /\b(onclick|onhover|onfocus|onblur|onchange|oninput|onkeydown|onkeyup|hover|focus|active|disabled):/g,
  ...
  // Properties
  html = html.replace(
    /\b(bg|col|pad|margin|w|h|gap|rad|fs|weight|center|hor|ver|spread|wrap|hidden|visible|name|placeholder|cursor)\b/g,
...
function highlightJsCode(code: string): string {
  ...
  // Keywords
  html = html.replace(
    /\b(const|let|var|function|async|await|return|if|else|for|while|import|export|from|default)\b/g,
    '<span class="syn-keyword">$1</span>'
  )
  return html
}
