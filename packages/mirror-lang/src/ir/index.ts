...
import type { IR, IRNode, IRStyle, IREvent, IRAction, IRProperty, IREach, IRConditional } from './types'
...
export function toIR(ast: AST): IR {
  const transformer = new IRTransformer(ast)
...
class IRTransformer {
  ...
  private transformInstance(instance: Instance | Each | any): IRNode {
    // Handle Each loops
    ...
    // Transform children with slot filling
  ...
  private transformEach(each: Each): IRNode {
    const eachData: IREach = {
      ...
      filter: each.filter,
      template: each.children.map(child => this.transformInstance(child)),
    }
  ...
  private transformConditional(cond: any): IRNode {
    const conditionalData: IRConditional = {
      ...
      then: cond.then.map((child: any) => this.transformInstance(child)),
      else: cond.else?.length > 0
        ? cond.else.map((child: any) => this.transformInstance(child))
        : undefined,
    }
   ...
   */
  private propertyToCSS(prop: Property): IRStyle[] {
    ...
    // Handle size property (size 100 200 or size 50 or size hug)
    if (name === 'size') {
      if (values.length === 1) {
