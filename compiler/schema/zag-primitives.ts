...
export interface ZagPrimitiveDef {
  ...
  pattern?: 'slots-only' | 'simple-items' | 'content-items' | 'repeating-items' | 'complex-nested' | 'nav-items'
...
export const ZAG_PRIMITIVES: Record<string, ZagPrimitiveDef> = {
  ...
  Form: {
    ...
    props: [
      ...
      'collection',
      // Auto-generation
      'auto',          // Auto-generate fields from schema
      // Validation
      'validateOnBlur', 'validateOnChange',
      // State
      'disabled', 'readOnly',
    ],
    events: ['onsubmit', 'onreset', 'onerror', 'onchange'],
    description: 'Form bound to collection with CRUD support',
    pattern: 'repeating-items',
    itemKeywords: ['Field'],
    itemProps: [
      'name',          // Field name (maps to schema field)
      'label',         // Display label
      'placeholder',   // Input placeholder
      'multiline',     // Use Textarea instead of Input
      'display',       // Display mode: 'input' | 'select' | 'checkbox' | 'switch' | 'slider'
      'filter',        // Filter function for relation selects
      'allowClear',    // Show clear button
      'max',           // Max length/value
      'required',      // Override schema required
...
export const SLOT_MAPPINGS: Record<string, Record<string, ZagSlotDef>> = {
  ...
  Form: {
    Root: { api: 'getRootProps', element: 'form' },
    Field: { api: 'getFieldProps', element: 'div', itemBound: true },
    FieldLabel: { api: 'getFieldLabelProps', element: 'label', itemBound: true },
    FieldInput: { api: 'getFieldInputProps', element: 'input', itemBound: true },
    FieldError: { api: 'getFieldErrorProps', element: 'span', itemBound: true },
 ...
 * Supports custom group keywords (e.g., 'NavGroup' for SideNav)
 */
export function isZagGroupKeyword(primitiveName: string, keyword: string): boolean {
  ...
  // Check for custom group keywords first
  if (primitivesDef.groupKeywords && primitivesDef.groupKeywords.includes(keyword)) {
    return true
  }
  // Check if Group is in the slots and keyword is 'Group'
