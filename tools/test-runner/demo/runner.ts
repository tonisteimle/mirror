...
export class DemoRunner {
  ...
  private async injectDemoAPI(config: DemoConfig): Promise<void> {
    ...
    // High-level Mirror actions API (E1, E2, B1, B2). The studio bundle
    // installs window.__mirrorActions for us via setupMirrorTestAPI; the
    // legacy inline-eval'd MIRROR_ACTIONS_API was removed once the bundled
    // module became the only path. If detection fails, the page is either
    // a non-studio target (no longer supported) or studio is older than
    // the bundle — surface that explicitly instead of running blind.
    const actionsBundled = await this.evaluate<boolean>(
      'typeof window.__mirrorActions === "object" && window.__mirrorActions !== null'
    )
    if (!actionsBundled) {
      throw new Error(
        '__mirrorActions not installed — studio bundle missing. Rebuild ' +
          'with `npm run build:studio` or load the page from the studio server.'
      )
    }
   ...
   * browser-side `drawInGrid` (in `studio/test-api/mirror-actions/`).
   */
