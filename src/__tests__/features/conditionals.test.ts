...
import type { ActionStatement } from '../../parser/types'
...
describe('V3: Conditionals and Iterators', () => {
  ...
  onclick assign $count to $count + 1`
      ...
      expect((action as ActionStatement).type).toBe('assign')
      expect((action as ActionStatement).target).toBe('count')

      const value = (action as ActionStatement).value
...
Content h $Header.h`
  ...
  onchange assign $inputValue to $event.value`
      ...
      const action = handler?.actions[0] as ActionStatement
  ...
  onclick assign $mouseX to $event.clientX`
      ...
      const action = result.nodes[0].eventHandlers?.[0]?.actions[0] as ActionStatement
