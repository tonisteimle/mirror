...
Button "Submit"`)
    ...
    it('implicit inheritance: first usage with properties creates reusable template', () => {
      // First instance with properties creates an implicit template
      // Subsequent usages inherit those properties
...
Myfooter`)
      ...
      // Second instance inherits properties AND children from implicit template
      const second = result.nodes[1]
      expect(second.name).toBe('Myfooter')
      expect(second.properties.pad).toBe(16)
      expect(second.properties.col).toBe('#242424')
...
Text $primary-col "Hello"`)
    ...
    it('infers col property from color value (value-based inference)', () => {
