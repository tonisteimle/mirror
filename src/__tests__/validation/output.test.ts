...
\`\`\`
--- COMPONENTS ---
Button: pad 12 background #3B82F6 border-radius 8

--- LAYOUT ---
Button pad 12 "Click me"
\`\`\`
...
This creates a styled button.`
  ...
  describe('Check Mode (No Corrections)', () => {
    it('reports errors without correcting', () => {
      ...
      // padding is now valid, use background instead (still needs correction)
      const result = validator.check(
        'Button: background #F00',
        'Button background #F00 "Click"'
      )

      // Should have errors/warnings but not correct them
      expect(result.components).toContain('background')
