...
const KEYWORD = /^(from|after|before|state|if|then|else|each|in|data|where|named|as|show|hide|toggle|open|close|page|animate|onclick|onhover|onchange|oninput|onload|onfocus|onblur|onkeydown|onkeyup|debounce|delay|highlight|select|deselect|clear-selection|filter|change|activate|deactivate|deactivate-siblings|toggle-state|assign|to|validate|reset|hover|focus|active|disabled|filled|alert|call|self|next|prev|first|last|first-empty|highlighted|selected|self-and-before|all|none|below|above|fade|scale|slide-up|slide-down|slide-left|slide-right|spin|pulse|bounce)\b/
// Doc-mode patterns
