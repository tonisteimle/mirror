...
│
├── bindings/              # Komplexe Komponenten mit Libraries
│   ├── datepicker/        # react-day-picker + date-fns
│   ├── dropdown/          # @radix-ui/react-dropdown-menu
│   ├── masked-input/      # react-imask
│   └── autocomplete/      # @radix-ui/react-popover
│
...
Bindings verbinden Mirror-Syntax mit professionellen JavaScript-Libraries.

### Architektur

```
Mirror Code → Parser → Binding Detection → Library Renderer
     ↑                        ↓
     └────── Styling ─────────┘
```
...
```typescript
interface Binding {
  name: string;                              // z.B. "Select"
  components: string[];                      // ["Select", "SelectOption", ...]
  validate: (input) => ValidationResult;     // Zod-basiert
  getDefaults: (density) => string;          // Mirror Code
  handles: (name) => boolean;                // Prüft Zuständigkeit
  transformNode: (node) => BindingInput;     // AST → Props
  Renderer: React.ComponentType;             // Library-Wrapper
}
```

### Implementierte Bindings

| Binding | Library | Komponenten |
|---------|---------|-------------|
| Select | @radix-ui/react-dropdown-menu | Select, SelectTrigger, SelectOption, ... |
| DatePicker | react-day-picker + date-fns | DatePicker, Calendar, CalendarDay, ... |
| MaskedInput | react-imask | MaskedInput, PhoneInput, IBANInput, ... |
| Autocomplete | @radix-ui/react-popover | Autocomplete, AutocompleteInput, ... |

### Flow

1. **Parser** erkennt Binding-Komponente via `isBindingComponent()`
2. **Binding** transformiert AST-Node zu Props via `transformNode()`
3. **Renderer** wrappt Library mit Mirror-Styles

Siehe `docs/BINDINGS.md` für Details.
...
│   └── datepicker.test.ts # DatePicker-spezifisch
